package top.nanli.weixin.service;
...
import top.nanli.weixin.entity.message.TextMessage;
import top.nanli.weixin.util.MessageType;
import top.nanli.weixin.util.XmlUtil;
    ...
    public static String parseMessage(Map<String, String> map) {
        ...
        try {
            ...
            if (MsgType.equals(MessageType.TEXT_MESSAGE)) {
                ...
                textMessage.setContent("您发送的是文本消息 " + map.get("Content"));
                ...
                replyMap.put("Content", new String(("您发送的是文本消息:" + map.get("Content")).getBytes(),"ISO-8859-1"));
                ...
                textMessage.setFromUserName(toUserName);
                textMessage.setCreateTime(new Date().getTime());
                textMessage.setContent("暂时只支持接受文本类型的消息!");
                textMessage.getMsgType();
                // respXml = ReplyMessageUtil.sendTextMessage(textMessage);

                // 用map集合封装
                replyMap.put("MsgType", MessageType.RESP_MESSAGE_TYPE_TEXT);
                replyMap.put("Content", "暂时只支持接受文本类型的消息!!");
                respXml = XmlUtil.xmlFormat(replyMap, true);
            }
